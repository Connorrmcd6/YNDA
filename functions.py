...
def render_svg_banner(
    path, width=None, height=None, gw_number=None, first_place_name=None, team_name=None
):
    with open(path, "r") as f:
        svg_content = f.read()
...
def render_svg_metric(
    path, width=None, height=None, name=None, metric_name=None, value_str=None
):
    with open(path, "r") as f:
        svg_content = f.read()
...
def render_svg_summary(
    path, width=None, height=None, ls=None, mps=None, rcs=None, ogs=None, nms=None
):
    with open(path, "r") as f:
        svg_content = f.read()
...
def update(_gc):
    ...
    events = pd.DataFrame(general_response["events"])[
        [
            "id",
            "is_previous",
            "is_current",
...
def get_illegible_nominees(df, current_gw):
    red_card_players = df[
        (df["event"] == current_gw) & (df["drink_type"] == "red card")
    ]["drinker_name"].tolist()
    own_goal_players = df[
        (df["event"] == current_gw) & (df["drink_type"] == "own goal")
    ]["drinker_name"].tolist()
    missed_pen_players = df[
        (df["event"] == current_gw) & (df["drink_type"] == "missed pen")
    ]["drinker_name"].tolist()
...
#         or (int(drinks["nomination_deadline_date"]) < current_unix_time)
#     ]["drinker_name"]
...
def build_rank_df(gameweek_df, current_week):
    last_10 = gameweek_df[gameweek_df.event >= current_week - 10][
        ["event", "player_name", "total_points"]
    ]
...
def build_laps(df):
    ...
    drinks_lap_times["nomination_completed_date"] = drinks_lap_times[
        "nomination_completed_date"
    ].apply(lambda x: create_unix(x))
    drinks_lap_times["nomination_deadline_date"] = drinks_lap_times[
        "nomination_deadline_date"
    ].apply(lambda x: create_unix(x))

    # Filter rows with valid nomination completion date
    drinks_lap_times = drinks_lap_times[
        drinks_lap_times["nomination_completed_date"]
        < drinks_lap_times["nomination_deadline_date"]
    ]

    # Compute the lap times
    drinks_lap_times["completion_time"] = np.round(
        (drinks_lap_times["end_time"] - drinks_lap_times["start_time"])
        * (330 / drinks_lap_times["drink_size"]),
        3,
    )
    ...
    fastest_times_df = pd.DataFrame(
        {"drinker_name": fastest_times.index, "completion_time": fastest_times.values}
    )
    ...
    fastest_times_df["gap"] = fastest_times_df["gap"].apply(
        lambda x: f"+{x:.3f}s" if pd.notna(x) and x != 0 else ""
    )
...
def categories(df):
    ...
    df["nomination_completed_date"] = df["nomination_completed_date"].apply(
        lambda x: create_unix(x)
    )
    df["nomination_deadline_date"] = df["nomination_deadline_date"].apply(
        lambda x: create_unix(x)
    )
...
def uno_reverse(gc, drinks, uno_data, sheet_key, nominee):
    try:
        ...
        filtered_drinks = filtered_drinks.sort_values(
            by="nomination_created_date", ascending=True
        )
        ...
        drinks.at[last_record_index, "drinker_name"] = filtered_drinks[
            "nominator_name"
        ][last_record_index]
    ...
    try:
        ...
        nomination_created_datetime = int(
            filtered_drinks.nomination_created_date.iloc[-1]
        )
        ...
        if current_timestamp > (nomination_created_datetime + two_days_in_seconds):
            raise Exception(
                f"You took more than 2 days to use your uno reverse card, try again next time{current_timestamp}; {(nomination_created_datetime + two_days_in_seconds)}"
            )
        else:
...
def managers_update(_gc, league_endpoint, current_gw):
    ...
    df1 = fetch_manager_data(
        _gc, managers_table, prod_google_sheet_key, numerical_columns
    )
...
def gameweek_results_update(_gc, current_gw, manager_details):
    ...
    for entry, player_name, entry_name in zip(
        manager_details["entry"],
        manager_details["player_name"],
        manager_details["entry_name"],
    ):
        ...
        new_gameweek_results = pd.concat(
            [new_gameweek_results, pd.DataFrame([new_player_row])], ignore_index=True
        )

        # Get player picks
        elements_out = [
            sub["element_out"] for sub in live_teams_response["automatic_subs"]
        ]
        elements_in = [
            sub["element_in"] for sub in live_teams_response["automatic_subs"]
        ]
        picks = live_teams_response["picks"]

        element_ids_modified = [
            pick["element"] for pick in picks if pick["element"] not in elements_out
        ]
        element_ids_modified.extend(elements_in)

        live_gameweek_endpoint = (
        ...
        red_card_flag, own_goal_flag, penalties_missed_flag = analyze_picks(
            element_ids_modified, live_gameweek_response
        )
        ...
        picks_data = pd.concat(
            [picks_data, pd.DataFrame([new_picks_data])], ignore_index=True
        )

    # Update google sheets for new_gameweek_results and picks_data
    write_google_sheets_data(
        _gc, new_gameweek_results, gameweek_results_table, prod_google_sheet_key
    )
    write_google_sheets_data(
        _gc, picks_data, gameweek_teams_table, prod_google_sheet_key
    )
...
def auto_assign_drinks(
    _gc, gameweek_results_table, gameweek_teams_table, prod_google_sheet_key, current_gw
):
    gameweek_results_table_df = fetch_google_sheets_data(
        _gc,
        gameweek_results_table,
        prod_google_sheet_key,
        ["event", "points", "total_points", "event_transfers_cost", "points_on_bench"],
    )
    gameweek_teams_table_df = fetch_google_sheets_data(
        _gc, gameweek_teams_table, prod_google_sheet_key, ["event"]
    )

    filtered_results = gameweek_results_table_df[
        gameweek_results_table_df["event"] == current_gw
    ]
    filtered_teams = gameweek_teams_table_df[
        gameweek_teams_table_df["event"] == current_gw
    ]

    lowest_points_player = filtered_results.sort_values(
        by=["points", "total_points", "player_name"]
    ).iloc[0]["player_name"]
    created_date = int(time.time()) + (
        60 * 60 * 2
    )  # add two hours to adjust for server time

    deadline_date_str = (datetime.now() + timedelta(days=7)).strftime(
        "%m/%d/%y %23:59:59"
    )
...
def can_nominate_flag(df, current_gw, first_place):
    return (
        len(df[(df["event"] == current_gw) & (df["nominator_name"] == first_place)])
        == 0
    )
...
def format_date(date_str):
    try:
        timestamp = int(
            date_str
        )  # Assuming date_str is a Unix timestamp in string format
