...
def categories(df):
    ...
    df["nomination_completed_date"] = pd.to_datetime(
        df["nomination_completed_date"], errors="coerce"
    )
    df["nomination_deadline_date"] = pd.to_datetime(
        df["nomination_deadline_date"], errors="coerce"
    )
...
def uno_reverse(gc, drinks, uno_data, sheet_key, nominee):
    ...
    try:
        ...
        nomination_created_datetime = datetime.strptime(
            filtered_drinks.nomination_created_date.iloc[0], "%d/%m/%y %H:%M:%S"
        )

        if (datetime.now() + timedelta(hours=2)) > (
            nomination_created_datetime + timedelta(days=2)
        ):
            raise Exception(
                "You took more than 2 days to use your uno reverse card, try again next time"
            )
        else:
...
def build_laps(df):
    ...
    drinks_lap_times["gap"] = drinks_lap_times["gap"].apply(
...
def auto_assign_drinks(
    ...
    deadline_date = (datetime.now() + timedelta(hours=2) + timedelta(days=7)).strftime(
        "%d/%m/%y %H:00"
    )
